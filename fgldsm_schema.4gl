...
FUNCTION sql_ext_pgs()
  ...
  PREPARE spgs_cdef FROM
    ...
    || " FROM PG_ATTRDEF AD, PG_CLASS C, PG_ATTRIBUTE A"
    || " WHERE C.OID = A.ATTRELID"
    || " AND AD.ADRELID = C.OID AND AD.ADNUM = A.ATTNUM"
    || " AND UPPER(PG_GET_USERBYID(C.RELOWNER)) = ?"
    || " AND C.RELNAME = ?"
    || " AND A.ATTNAME = ?"
  ...
  DECLARE cpgs_pkey CURSOR FROM
             ...
             || "  P.CONNAME,"
             || "  A.ATTNAME"
             || " FROM PG_CONSTRAINT P, PG_ATTRIBUTE A, PG_CLASS C"
             || " WHERE P.CONTYPE='p'"
  ...
  DECLARE cpgs_fkey CURSOR FROM
             ...
             || "  F.CONFDELTYPE,"
             || "  F.CONFUPDTYPE"
             || " FROM PG_CONSTRAINT F, PG_CLASS C, PG_CONSTRAINT K, PG_CLASS R"
             || " WHERE F.CONTYPE='f'"
  ...
  DECLARE cpgs_chk CURSOR FROM
             ...
             || "  K.CONNAME,"
             || "  K.CONSRC"
             || " FROM PG_CONSTRAINT K, PG_CLASS C"
             || " WHERE K.CONTYPE='c'"
